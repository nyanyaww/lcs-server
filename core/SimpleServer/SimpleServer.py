...
class SimpleServer(object):
            ...
            if client == self.server:
                ...
                try:
                    ...
                    if receive_data:
                        ...
                        # 将收到的消息放入到各客户端的消息队列中
                        # 将数据解析成哈希表
                        send_data, client_name = self.message_handle(receive_data, client)
                        self.client_name_dict[client_name] = client
                        # 将回复操作放到output列表中，让select监听
                        self.receive_message_queue[self.client_name_dict[client_name]].put(
                            send_data)
                        if client_name not in self.output_list:
                            self.output_list.append(self.client_name_dict[client_name])
    ...
    def message_handle(self, receive_data, client):
        return receive_data, str(client)
        ...
        for client in self.output_list:
            try:
                ...
                if not self.receive_message_queue[client].empty():
                    ...
                    print('client_name:{0}'.format(get_key(self.client_name_dict, client)[0]))
                    print('client:{0}\nsend_data:{1}'.format(client, send_data))
                else:
                    # 将监听移除等待下一次客户端发送消息
...
if __name__ == "__main__":
    raspberry_server = SimpleServer(8086)
